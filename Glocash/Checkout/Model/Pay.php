...
class Pay extends \Magento\Payment\Model\Method\AbstractMethod
{
	...
	public function getGlocashUrl($quote){
		$param= array(
				//付款请求参数
				'REQ_TIMES'=>time(),
				...
				'URL_SUCCESS'=>$this->helper->getUrl('glocash/standard/responsepay')."?REQ_INVOICE=".$quote->getReservedOrderId().'&CUS_EMAIL='.$quote->getCustomerEmail(),
				'URL_PENDING'=>$this->helper->getUrl('glocash/standard/responsepay')."?REQ_INVOICE=".$quote->getReservedOrderId().'&CUS_EMAIL='.$quote->getCustomerEmail(),
				'URL_FAILED'=>$this->helper->getUrl('checkout'),
		...
		if ($httpCode!=200 || empty($datas['URL_PAYMENT'])) {
			// 请求失败
			$action='';
			Logs::logw("Request connection failed \n url:".$gatewayUrl." method:post Request:".json_encode($param)." Result:".$result,"glocash.log","payment_url");
		}
		else{
			$action=$datas['URL_PAYMENT'];
			Logs::logw("url:".$gatewayUrl." method:post Request:".json_encode($param)." Result:".$result,"glocash.log","payment_url");
		}
    ...
    public function postProcessing(\Magento\Sales\Model\Order $order, \Magento\Framework\DataObject $payment, $response) {
		...
		Logs::logw("Successful modification of order status,ID:".$order->getId()." Number:".$response['REQ_INVOICE']." Status:".$this->getOrderStatus(),"glocash.log","Order_modification");
